...
if ( ! class_exists( 'Education_Web_Info' ) ) {
	...
	class Education_Web_Info {
		 ...
		 * Single instance.
		 *
		 * @var Education_Web_Info $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Education_Web_Info ) ) {
				self::$instance = new Education_Web_Info;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( '%s Info', 'education-web' ), $this->theme_name );
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( '%s Info', 'education-web' ), $this->theme_name );
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->page_slug     = $this->theme_slug . '-info';
			$this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
			$this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme info page.', 'education-web' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'education-web' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'education_web_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'education-web' ) . '</a></p>';
		}
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_education_web_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_education_web_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_page() {
			?>
			<div class="wrap about-wrap eduweb-info-wrap">
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'education-web' ), $method );
			}
		}
		...
		public function getting_started() {
			...
			?>
			<div class="feature-section eduweb-section eduweb-section-getting-started three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .eduweb-section -->
			<?php
		}
		...
		public function support() {
			...
			?>
			<div class="feature-section eduweb-section eduweb-section-support three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .eduweb-section -->
		...
		public function useful_plugins() {
			...
			?>
			<div class="eduweb-section eduweb-section-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'education-web' ); ?></a>
				<?php endif; ?>
		...
		public function demo_content() {
			...
			?>
			<div class="eduweb-section eduweb-section-demo-content">
		...
		public function upgrade_to_pro() {
			...
			?>
			<div class="eduweb-section eduweb-section-upgrade-to-pro">
		...
		public function assets( $hook ) {
			...
			if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
				wp_enqueue_style( 'education-web-info', get_template_directory_uri() . '/offshorethemes/admin/about-theme/css/info' . $min . '.css', array(), '1.0.0' );
				wp_enqueue_script( 'education-web-info', get_template_directory_uri() . '/offshorethemes/admin/about-theme/js/info' . $min . '.js', array(), '1.0.0' );
			}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta( $user_id, 'education_web_dismiss_status', true );
			?>
			<?php if ( current_user_can( 'edit_theme_options' ) && 'themes' === $screen_id && 1 !== absint( $dismiss_status ) ) : ?>
				<div class="eduweb-info-notice notice notice-info">
					<?php $this->render_notice(); ?>
				</div><!-- .eduweb-info-notice -->
			<?php endif; ?>
			<?php
		}
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'education_web_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'education_web_dismiss_status', 1 );
